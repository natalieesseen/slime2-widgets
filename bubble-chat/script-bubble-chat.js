...
addEventListener('slime2:widget-data-update', () => {
  ...
  customBadges = {
    broadcaster: badges.broadcasterBadge,
    moderator: badges.modBadge,
    vip: badges.vipBadge,
    founder: badges.founderBadge,
    'artist-badge': badges.artistBadge
  }

  // pronouns settings
  eventList.addClass(`pronouns-${pronouns.display}`)

  // bubble chat - specific settings
  if (bubblechat.colorMode !== previousMode || bubblechat.theme !== previousTheme) {
    let updatedBubbleChat = { ...bubblechat };
    let updatedTextStyles = { ...textStyles };
    ...
    const themeColors = {
      pink: { light: '#f5c2e7', dark: '#f38ba8' },
      orange: { light: '#fab387', dark: '#ef9f76' },
      teal: { light: '#94e2d5', dark: '#81c8be' },
  ...
  eventList.css({
    '--nameBg': bubblechat.nameBg,
    '--msgBg': bubblechat.msgBg,
...
function buildUser(user) {
  ...
  if(buildBadges(badges).length > 0) {
    userClone.find('.namePronouns').after('<span class="badges"></span>')
...
function buildBadges(badges) {
  return badges.map(badge => {
    const { id, image } = badge

    console.log(badge)

    const badgeClone = $(slime2.cloneTemplate('badge-template'))

    if (customBadges && customBadges[id]) {
      badgeClone.find('.badge').attr('src', customBadges[id])
    } else {
